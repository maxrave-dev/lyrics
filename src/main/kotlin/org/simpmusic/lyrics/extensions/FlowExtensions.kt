 ...
 */
fun <T> Flow<T>.logEach(prefix: String): Flow<T> =
    this.onEach {
 ...
 */
fun <T> Flow<T>.logError(prefix: String): Flow<T> =
    this.catch { e ->
 ...
 */
fun <T> Flow<T>.logCompletion(message: String): Flow<T> =
    this.onCompletion {
 ...
 */
fun <T> Flow<T>.shareHot(
    scope: CoroutineScope,
    replay: Int = 1,
): Flow<T> = this.shareIn(scope, SharingStarted.Eagerly, replay)
