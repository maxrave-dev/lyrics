...
class AppwriteNotFoundTranslatedLyricRepository(
    ...
    override fun findById(id: String): Flow<Resource<NotFoundTranslatedLyric?>> = flow {
    ...
    override fun findByVideoIdAndLanguage(videoId: String, language: String): Flow<Resource<NotFoundTranslatedLyric?>> = flow {
        ...
        runCatching {
            ...
            onSuccess = { documents ->
                val notFoundTranslatedLyric = documents.documents.firstOrNull()?.let { documentToNotFoundTranslatedLyric(it) }
                logger.debug("findByVideoIdAndLanguage --> Found notfound translated lyric for videoId: $videoId, language: $language: ${notFoundTranslatedLyric != null}")
                emit(Resource.Success(notFoundTranslatedLyric))
            },
            onFailure = { e ->
                logger.error("findByVideoIdAndLanguage --> Error finding notfound translated lyric by videoId: $videoId, language: $language", e)
                emit(Resource.Error("Failed to find notfound translated lyric by videoId and language: ${e.message}", e as? Exception))
            }
        )
        logger.debug("findByVideoIdAndLanguage --> findByVideoIdAndLanguage completed for videoId: $videoId, language: $language")
    }.flowOn(Dispatchers.IO)

    override fun findBySha256Hash(sha256hash: String): Flow<Resource<NotFoundTranslatedLyric?>> = flow {
        ...
        runCatching {
            ...
            onSuccess = { documents ->
                val notFoundTranslatedLyric = documents.documents.firstOrNull()?.let { documentToNotFoundTranslatedLyric(it) }
                logger.debug("findBySha256Hash --> Found notfound translated lyric for sha256hash: $sha256hash: ${notFoundTranslatedLyric != null}")
                emit(Resource.Success(notFoundTranslatedLyric))
            },
            onFailure = { e ->
                logger.error("findBySha256Hash --> Error finding notfound translated lyric by sha256hash: $sha256hash", e)
                emit(Resource.Error("Failed to find notfound translated lyric by sha256hash: ${e.message}", e as? Exception))
            }
        )
        logger.debug("findBySha256Hash --> findBySha256Hash completed for sha256hash: $sha256hash")
    }.flowOn(Dispatchers.IO)

    override fun findAllOrderedByDate(limit: Int?, offset: Int?): Flow<Resource<List<NotFoundTranslatedLyric>>> = flow {
        ...
        runCatching {
            ...
            onSuccess = { documents ->
                logger.debug("findAllOrderedByDate --> Successfully found ${documents.documents.size} notfound translated lyrics ordered by date")
    ...
    override fun save(notFoundTranslatedLyric: NotFoundTranslatedLyric): Flow<Resource<NotFoundTranslatedLyric>> = flow {
        ...
        runCatching {
            ...
            onSuccess = { document ->
                logger.debug("save --> Successfully saved notfound translated lyric with sha256hash: ${notFoundTranslatedLyric.sha256hash}")
                emit(Resource.Success(documentToNotFoundTranslatedLyric(document)))
            },
            onFailure = { e ->
                logger.error("save --> Error saving notfound translated lyric with sha256hash: ${notFoundTranslatedLyric.sha256hash}", e)
    ...
    override fun deleteByVideoIdAndLanguage(videoId: String, language: String): Flow<Resource<Boolean>> = flow {
        ...
        runCatching {
            ...
            logger.debug("deleteByVideoIdAndLanguage --> Finding notfound translated lyric document for videoId: $videoId, language: $language")
            databases.listDocuments(
                databaseId = databaseId,
                collectionId = collectionId,
                queries = listOf(
            ...
            onSuccess = { documents ->
                if (documents.documents.isNotEmpty()) {
                    ...
                    runCatching {
                        logger.debug("deleteByVideoIdAndLanguage --> Deleting notfound translated lyric document with documentId: $documentId")
                        ...
                        onSuccess = {
                            logger.debug("deleteByVideoIdAndLanguage --> Successfully deleted notfound translated lyric for videoId: $videoId, language: $language")
                            emit(Resource.Success(true))
                        },
                        onFailure = { e ->
                            logger.error("deleteByVideoIdAndLanguage --> Error deleting notfound translated lyric for videoId: $videoId, language: $language", e)
                            emit(Resource.Error("Failed to delete notfound translated lyric: ${e.message}", e as? Exception))
                        }
                    )
                } else {
                    logger.debug("deleteByVideoIdAndLanguage --> No notfound translated lyric found for videoId: $videoId, language: $language")
                    emit(Resource.Success(false))
                }
            },
            onFailure = { e ->
                logger.error("deleteByVideoIdAndLanguage --> Error finding notfound translated lyric for videoId: $videoId, language: $language", e)
                emit(Resource.Error("Failed to find notfound translated lyric for deletion: ${e.message}", e as? Exception))
            }
        )
    ...
    private fun documentToNotFoundTranslatedLyric(document: Document<Map<String, Any>>): NotFoundTranslatedLyric {
        return NotFoundTranslatedLyric(
