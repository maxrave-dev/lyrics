...
class CoroutineConfig {
    ...
    fun applicationScope(@Qualifier("ioDispatcher") ioDispatcher: CoroutineDispatcher): CoroutineScope = 
    ...
    fun dispatcherBeanPostProcessor(): DestructionAwareBeanPostProcessor {
        return object : DestructionAwareBeanPostProcessor {
            @Throws(BeansException::class)
            override fun requiresDestruction(bean: Any): Boolean {
                return bean !is NonCloseableCoroutineDispatcher && bean is CoroutineDispatcher
            }
 ...
 */
class NonCloseableCoroutineDispatcher(private val delegate: CoroutineDispatcher) : CoroutineDispatcher() {
 ...
 */
class CloseableCoroutineScope(private val context: kotlin.coroutines.CoroutineContext) : 
